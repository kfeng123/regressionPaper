...
there is another line of research utilizing desparsified Lasso estimator; see, e.g., \cite{Dezeure2017} and \cite{ZC2017}.
...
Hence it would be better to give a critical value of $T$ which can preserve the test level under general distributions of $\bepsilon$.
...
\begin{proposition}
    ...
    then
    \begin{equation*}
        \frac{Z^\top \BA Z + \Bb^\top Z - \mytr(\BA)}{\sqrt{2\mytr(\BA^2)+\|\Bb\|^2}}
...
\begin{proof}[\textbf{Proof of Theorem \ref{thm:criticalValue}}]
    ...
    Consequently, $F^\dagger(x)$ is continuous and is strict increasing for $x\in\{x:0<F(x)<1\}$; see \cite{Sevast1961A} as well as the remark made by A. N. Kolmogorov on that paper.
...
\begin{proof}[\textbf{Proof of Proposition \ref{Lemma:normal}}]
    ...
    Hence we can assume $\lim_{n\to \infty}\|\Bb\|^2/\mytr(\BA^2) =c \in [0,+\infty]$.
