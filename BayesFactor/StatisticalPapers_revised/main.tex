...
\begin{proposition}
    ...
    Suppose $(n-q)$ is divisible by $3$ and $\gamma_1 = \dots=\gamma_{(n-q)/3}= 4 p$, $\gamma_{(n-q)/3 + 1} = \dots= \gamma_{2(n-q)/3} = 2 p $, $\gamma_{(2(n-q)/3 + 1} = \dots = \gamma_{n-q} = p$ where $\breve \gamma_1 > \breve \gamma_2 > \breve \gamma_3 > 0$ are absolute constants and $c_n>0$ may depend on $n$.
