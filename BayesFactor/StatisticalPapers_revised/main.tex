...
It is hard to compare the two tests in general as the forms of the global power functions are fairly complicated.
Nevertheless, the forms of the local power functions can be further simplified under more restricted assumptions.
...
\begin{proof}[\textbf{Proof of Proposition \ref{new:proposition:touteng}}]
    ...
    From Theorem \ref{generalTheorem}, we have
\begin{equation*}
    \begin{split}
    &\Pr\left( 
        \frac{
...
\sqrt{2 \myVar\left( \gamma_I^k \right)}
    ...
    \end{split}
\end{equation*}
We only need to show that $\myE(\gamma_I w_I^2) = o(1)$ and 
...
%To facilitate the conparison of the two tests, we would like to impose more assumptions and simplify the power functions.




\begin{proposition}
    Suppose model \eqref{label:linearModel} holds with $\bepsilon\sim \mathcal N_n (0, \BI_n)$.
    Suppose $(n-q)$ is divisible by $3$ and $\gamma_1 = \dots=\gamma_{(n-q)/3}= 4 p$, $\gamma_{(n-q)/3 + 1} = \dots= \gamma_{2(n-q)/3} = 2 p $, $\gamma_{(2(n-q)/3 + 1} = \dots = \gamma_{n-q} = p$.
        Suppose as $n\to \infty$, $n-q\to \infty$ and $ \| \BV_b^{*\top} \bbeta_b \|^2 = O((n-q)^{1/2}/ p ) $.
        Then the asymptotic power of the proposed test is
        \begin{equation*}
            \Phi \left(
                \Phi^{-1}(\alpha) + 
                \frac{p}{\sqrt{28(n-q)}}\left(16\sum_{i=1}^{\frac{n-q}{3}} w_i^2  + 2 \sum_{i=\frac{n-q}{3}+1}^{\frac{2(n-q)}{3}} w_i^2 - 5 \sum_{i=\frac{2(n-q)}{3}+1}^{n-q} w_i^2 \right) 
            \right)
            ;
        \end{equation*}
    \label{prop:addedInRevision}
        ...
        \begin{equation*}
            \Phi \left(
                \Phi^{-1}(\alpha) + 
                \frac{p}{\sqrt{28(n-q)}}\left(20\sum_{i=1}^{\frac{n-q}{3}} w_i^2 -2 \sum_{i=\frac{n-q}{3}+1}^{\frac{2(n-q)}{3}} w_i^2 - 4 \sum_{i=\frac{2(n-q)}{3}+1}^{n-q} w_i^2 \right) 
            \right)
            .
        \end{equation*}
\end{proposition}

\begin{proof}[\textbf{Proof of Proposition \ref{prop:addedInRevision}}]
    It is straightforward to show that form fixed $k\neq 0$,
    \begin{equation*}
    \frac{
        \max_{1\leq i \leq n-q}
        \left( 
        ...
        (n-q) \myVar (\gamma_I^k)
    } = O\left(\frac{1}{n-q}\right)\to 0.
    \end{equation*}
    Hence Theorem \ref{generalTheorem} holds.
    Then the asymptotic power function of the proposed test is
    \begin{align*}
        \myE(\gamma_I w_I^2)  =
        \frac{p}{n-q}  \left(\breve \gamma_1 \sum_{i=1}^{\frac{n-q}{3}} w_i^2
        +
         \breve \gamma_2 \sum_{i=\frac{n-q}{3}+1}^{\frac{2(n-q)}{3}} w_i^2
        +
    \breve \gamma_3 \sum_{i=\frac{2(n-q)}{3}+1}^{n-q} w_i^2\right),
    \end{align*}
    \begin{align*}
    \myCov(\gamma_I^k, \gamma_I w_I^2) =
    \frac{p^{k+1}}{n-q}\Bigg( & (\breve \gamma_1^k- \myE(\gamma_I^k)) \breve \gamma_1 \sum_{i=1}^{\frac{n-q}{3}} w_i^2
        +\\
        &
        (\breve \gamma_2^k - \myE(\gamma_I^k) ) \breve \gamma_2 \sum_{i=\frac{n-q}{3}+1}^{\frac{2(n-q)}{3}} w_i^2
        +
        \\
        &
    (\breve \gamma_3^k - \myE(\gamma_I^k) )\breve \gamma_3 \sum_{i=\frac{2(n-q)}{3}+1}^{n-q} w_i^2
\Bigg).
    \end{align*}
...
It can be seen that the power of the proposed test mainly depends on $\myCov (-\gamma_I^{-1}, \gamma_I w_I^2)$ while the power of the test of \cite{Goeman2006} mainly depends on $\myCov(\gamma_I, \gamma_I w_I^2)$.  
