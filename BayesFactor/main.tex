...
\begin{document}
\title{
Nonexistence of unbiased test for high-dimensional linear model with fixed design and a Bayesian-motivated test 
}
...
The test statistic of Goeman {\rm et al.} \cite{Goeman2006} is obtained by letting the prior magnitude tend to zero.
...
However, the situation is different for our problem.
In fact, Theorem \ref{prop:unbiased} shall show that no test can detect all large deviations from the null hypothesis in large $p$, small $q$ setting.
Consequently, it is not enough to consider only the local alternative.
In this paper, we propose a new test statistic in large $p$, small $q$ setting which is the limit of Bayes factors as the prior magnitude tend to infinity.
...
%Then the following results hold.
    ...
    If $\lambda(\{\By:\varphi(\By)\neq \alpha\})>0$, then for every $M\geq 0$, there exists a parameter $\theta=(\bbeta_a^\top,\bbeta_b^\top,\phi)^\top \in \Theta$ such that $\phi\bbeta_b^\top \BX_b^\top \tilde \BP_a \BX_b \bbeta_b > M$ and $\myE_\theta [\varphi(\By)]< \alpha$.
\end{theorem}
It follows from Theorem \ref{prop:unbiased} that there is no nontrivial unbiased test in large $p$, small $q$ setting.
But Theorem \ref{prop:unbiased} provides more information.
...
See Sevast'yanov \cite{Sevast1961A} for a full characterization of the limit distributions of quadratic forms of normal random variables.
...
Bai {\rm et al.} \cite{Bai2017} gave a consistent estimator of $\phi^2 \myE (\epsilon_1^4)-1$  based on the standardized residuals.
