...
Consequently, compared with the test of \cite{Goeman2006}, the proposed test can always detect the signals from more directions.
In some important situations, the difference $d_1-d_2$ can be very large.
For example, suppose all predictors in $\BX_b$ are affected by a latent factor.
In this case, $\gamma_1$ is much larger than $\gamma_2,\ldots,\gamma_{n-q}$; see, e.g., \cite{Fan2013Large}.
...
On the other hand, the rows of $\BX_b$ are iid from $\mathcal N(0,\bSigma_b)$ and three structures of $\bSigma_b$ are considered, namely, Identity covariance matrix: $\bSigma_b=\BI_p$; Toeplitz structure: $(\bSigma_b)_{i,j}=0.9^{|i-j|}$; equal correlation structure: $(\bSigma_b)_{i,j}=0.1$ if $i\neq j$ and $(\bSigma_b)_{i,j}=1$ otherwise.
...
From Figure \ref{fig:1}, we can see that in the cases where $\bSigma_b =\BI_p$ and $n=50$, the powers of all three tests tend to $0$ as SNR increases.
This phenomenon, which is not surprising in view of Proposition \ref{prop:unbiased}, disappears when the sample size rises to $100$.
In the cases where $\bSigma_b = \BI_p$ and $n=100$, the EP test performs the best among the three tests while the proposed test and the GT test have similar performance.
Figure \ref{fig:2} shows that in the cases where $\bSigma_b$ has Toeplitz structure, all three tests have good performance.
As illustrated in Figure \ref{fig:3},  when $\bSigma_b$ has equal correlation structure, the proposed test is more powerful than its two compititors.
...
\begin{figure}
    ...
    \caption{Box plots of the empirical powers based on $100$ independently generated $\bbeta_b$.
$\bSigma_b$ has Toeplitz structure.
    }\label{fig:2}
\end{figure}
...
\begin{figure}
    ...
    \caption{Box plots of the empirical powers based on $100$ independently generated $\bbeta_b$.
    $\bSigma_b$ has equal correlation structure.
    }\label{fig:3}
\end{figure}
