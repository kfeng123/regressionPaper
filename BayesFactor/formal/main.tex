...
\end{equation*}
Then the proposed test rejects the null hypothesis when $R_{-1}$ is small and the test in \cite{Goeman2006} rejects the null hypothesis when $R_{1}$ is large.
The constant term $
-
{\mytr  (\BX_b^* \BX_b^{*\top})^{k}}/{(n-q)}
$ makes $\myE R_k=0$ under the null hypothesis.
Note that
\begin{equation*}
    R_k=
    \frac{
...
\end{equation*}
The following proposition is the key to investigate the asymptotic.
\begin{proposition}
...
Define $w_i=(\BV_b^{*\top} \bbeta_b)_i$, $i=1,\ldots, n-q$.
...
%\begin{equation*}
    ...
    %\frac{
        %\sqrt{n-q}
          %\|\bbeta_b\|^2 \phi \sigma_b^{2} \left(\myE \eta^{k+1} - \myE \eta^k \myE \eta\right)
    %}{\sqrt{
            %2
            %\myE (\eta^k - \myE \eta^k)^2
    %}} 
...
+
            4\phi
                \myE \left(( \gamma_I^{k} - \myE \gamma_I^{k})^2 \gamma_I w_I^2 \right)
    }
} 
\right)
+o(1).
...
From Theorem \ref{generalTheorem}, the asymptotic power function of the proposed test is
\begin{equation*}
    \begin{split}
    &
    \Pr\left( 
...
+
            4\phi
                \myE \left(( \gamma_I^{-1} - \myE \gamma_I^{-1})^2 \gamma_I w_I^2 \right)
    }
} 
\right)
+o(1).
...
The asymptotic power function of their test is
\begin{equation*}
    \begin{split}
    &
    \Pr\left( 
...
+
            4\phi
                \myE \left(( \gamma_I - \myE \gamma_I)^2 \gamma_I w_I^2 \right)
    }
} 
\right)
+o(1).
...
Let $I$ be a random variable uniformly distributed on $\{1,\ldots, n-q\}$.
\begin{equation*}
    \begin{split}
        A_3=&\bbeta_b^\top  \BV_b^* 
        \left( 
        \BD_b^{*2k+2}
        -\frac{\mytr (\BD_b^{*2k})}{n-q}  \BD_b^{*2}
        \right)
         \BV_b^{*\top} \bbeta_b
         \\
         =&
         \sum_{i=1}^{n-q}\gamma_i^{k+1} w_i^2
         -\frac{1}{n-q} \sum_{i=1}^{n-q} \gamma_i^{k} \sum_{i=1}^{n-q} \gamma_i w_i^2
         =
         (n-q)  
             \myCov \left(\gamma_I^k, \gamma_I w_I^2\right)
    .
...
\begin{equation*}
    \begin{split}
        \myE (\By^{*\top} \By^*  ) =& \bbeta_b^\top \BX_b^{*\top}  \BX_b^* \bbeta_b
    + \phi^{-1}(n-q),
    \\
    \myVar ( \By^{*\top} \By^*  ) 
    \leq  &
...
+ n-q\right)
\\
=&o\left( \left( \myE \By^{*\top} \By^*   \right)^2 \right).
    \end{split}
\end{equation*}
Consequently,
\begin{equation*}
...
\right)
=
(1+o_P(1)) (n-q) \left( \myE \gamma_I w_I^2 + \phi^{-1}  \right)
.
\end{equation*}
...
Then the conclusion follows from \eqref{eq:to2}, \eqref{eq:to1}, \eqref{eq:to3} and the following facts
\begin{align*}
        \bbeta_b^\top
        \BX_b^{*\top}
        \BB
        ...
        (n-q)\left( 
            \myCov\left( \gamma_I^k, \gamma_I w_I^2 \right)
            -
            \sqrt{\frac{2\myVar\left( \gamma_I^k \right)}{n-q}} x
            \myE (\gamma_I w_I^2)
        \right),
    \\
        \bbeta_b^\top
