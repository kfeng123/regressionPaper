...
\begin{abstract}
    ...
    We propose a test statistic which is the limit of Bayes factors under normal distribution.
    The null distribution of the test statistic is approximated by Lindeberg's replacement trick.
    We give the explicit error bound of this approximation.
...
Under normal error assumption, we also derive the asymptotic power function of the proposed test and the test of \cite{Goeman2006}.
...
Define
    ...
    \end{equation*}
    where $\BA^{\circ 2}=\BA\circ \BA$ and $\circ$ is Hadamard product.
...
\begin{proposition}\label{prop:estimation}
    Suppose the conditions of Theorem \ref{TheoremLindeberg} hold for $\bxi=\sqrt \phi \bepsilon$.
    Suppose $q/n\to 0$.
...
In the first simulation, the rows of $\BX_b$ are independently generated from $\mathcal N(0,\bSigma)$ where $\bSigma$ has Toeplitz structure $(\bSigma)_{i,j}=0.8^{|i-j|}$.
...
\begin{proof}[\textbf{Proof of Proposition \ref{prop:unbiased}}]
    ...
    We claim that the conclusion holds provided $\varphi(\By) \geq \alpha$, a.s.\ $\lambda$.
